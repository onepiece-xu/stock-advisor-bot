...
class MonitorRuntime:
    ...
    def run_once(self) -> None:
        ...
        for stock in self.config.monitor.stocks:
            ...
            if self._should_notify(stock.symbol, result):
                self._notify(stock.symbol, result.title, result.message)
    ...
    def _should_notify(self, symbol: str, result) -> bool:
        if not self.config.monitor.notification.feishu.enabled or not self.config.monitor.notification.feishu.webhook_url:
            return False
